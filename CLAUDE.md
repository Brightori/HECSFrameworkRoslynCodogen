...
Standalone-кодогенератор на Roslyn для HECS Framework. Консольное .NET 7 приложение: парсит `.cs` файлы целевого проекта (Unity-клиент или сервер) и генерирует boilerplate — карты типов, битовые маски, биндинги систем, резолверы сериализации, Unity-blueprint'ы, карту сетевых команд.
...
| `RoslynHECS/CodogeneratorRoslynPart.cs` | ~3100 строк, все генераторы, `partial class CodeGenerator` |
| `RoslynHECS/EntitiesWorld.cs`, `FastWorldPart.cs` | ещё две части `CodeGenerator` |
...
3. **`Program.componentsDeclarations` — упорядоченный список.** Позиция = бит в `HECSMask` и индекс в `TypesProvider`. Любое изменение порядка ломает совместимость сейвов и сетевого протокола.
...
5. **Инварианты генерат↔рантайм.** Нельзя менять: алгоритм `IndexGenerator.GetIndexForType`, раскладку маски (константы `61` в `ComponentsCountRoslyn` и `63` в `CalculateIndexesForMaskRoslyn`), сигнатуры `partial void FillRegistrators()` / `FillTypeRegistrators()`, имена `ComponentProviderRegistrator<T>`, `TypeRegistrator<T>`, `ComponentBluePrintContainer<T>`, `SystemBluePrint<T>`, `FastComponentMonoProvider<T>`.
